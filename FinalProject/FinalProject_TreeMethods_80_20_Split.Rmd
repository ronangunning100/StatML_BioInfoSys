---
title: "FinalProject_TreeMethods_80_20_Split"
...
This markdown file contains our exploration and analysis of performance of the following tree methods for the 80/20 split:
...
# Create split
sample <- sample(1:nrow(full_data), size = nrow(full_data)*0.8, replace = FALSE)
...
# tree method with no additional corrections has a test accuracy of 86.41% error. Improve this via cross validation? ...
...
cat("\n Based on the output from cv.tree.trainmodel, it would appear that pruning does not meaningfully improve performance of the tree (e.g. k=-Inf for model size of 9 terminal nodes with a deviance of 1035 while k=0 (minimal pruning) has model size of 6 and has a deviance of 1035.\n")

# It's apparent overfitting is not the issue truly the issue. While there is marginal improvement in deviance from 9 terminal nodes to 6 nodes (1035 to 1030 deviance), it's clear (as it was in the 70/30 split) that the issue is more so traversing the featurespace. So let's try bagging.
```
...
# Bagging test accuracy for 80/20 split is 97.67%
```
...
# As can be seen, by randomly sampling the features to be considered at each split, we limit the domination of strong predictors, maintain variance/overfit reduction, and thereby improve test accuracy from 97.67% to 97.91%. However, let's see if limiting the maximimum number of terminal nodes can improve RF test accuracy further...
```
...
# NOTE: train() didn't support tuning maxnode parameter in the tuning grid, hence running multiple chunks of the randomForest() command with different maxnode values.
...
# Resulting Accuracy: 98.20% ## BEST ONE --> same as in 70/30
...
# Resulting Accuracy: 97.86%

stopCluster(c1)


# As can be seen, the test accuracy for RF of the 80/20 split is improved by reducing the maximimum number of terminal nodes improves as it lowers overfit. Test accuracy goes from 97.91% --> 98.20%
...
# bm  shrinkage= 0.01    Accuracy: 94.02913 %
# bm0 shrinkage= 0.3     Accuracy: 98.59223 %
# bm1 shrinkage= 0.31    Accuracy: 98.88350 %
# bm2 shrinkage= 0.315   Accuracy: 98.30097 %
# bm3 shrinkage= 0.32    Accuracy: 98.39806 %
# bm4 shrinkage= 0.325   Accuracy: 99.02913 % # BEST PERFORMING
# bm5 shrinkage= 0.33    Accuracy: 98.49515 %
# bm6 shrinkage= 0.335   Accuracy: 98.34951 %
# bm7 shrinkage= 0.34    Accuracy: 98.49515 %
# bm8 shrinkage= 0.345   Accuracy: 98.49515 %
# bm9 shrinkage= 0.35    Accuracy: 98.49515 %
# bm10 shrinkage= 0.355  Accuracy: 98.64078 %
# bm11 shrinkage= 0.36   Accuracy: 98.44660 %
# bm12 shrinkage= 0.365  Accuracy: 98.44660 %
# bm13 shrinkage= 0.37   Accuracy: 98.44660 %
# bm14 shrinkage= 0.29   Accuracy: 98.78641 %
# bm15 shrinkage= 0.295  Accuracy: 98.78641 %
# bm16 shrinkage= 0.2975 Accuracy: 98.54369 %
# bm17 shrinkage= 0.305  Accuracy: 98.73786 %
# on previous run these values were obtained
```

## Evaluating Generative Boosting Accuracy
```{r}
boost.test.predict <- predict(boost_model4, test_data, n.trees = 500, type = "response")
...
cat("\nGenerative Boost Test Accuracy for: boost_model4 (best performing)\n") # update if bm# changes
...
X_train_matrix <- as.matrix(as.matrix(train_data[,3:563]))
X_test_matrix <- as.matrix(as.matrix(test_data[,3:563]))
...
# Test Accuracy found on run: 99.37%
```
